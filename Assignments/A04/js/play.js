var play = {
	create: function () {
		...
		// Death sound
		this.sound.kill = game.add.audio('exp')
		...
		//this.fireButton = this.input.keyboard.addKey(Phaser.Keyboard.DOWN);
	...
	update: function () {
		...
		spawn_rate = 70-game.global.score;	// how fast to add new obstacles to screen (smaller value = more obstacles)
	...
	spawnObstacle: function (x, y, speed) {
		var list=['icon-blue','icon-green','icon-chicken','icon-poop','icon-toffee','icon-minion','icon-cookie'];
		// randomly choose an icon from an array of icon names
		var choice = game.rnd.integerInRange(0, list.length-1);
		var name = list[choice];
	...
	destroyItem: function(bullet, obstacle){
		...
		//increases points on difficulty increases
		if(spawn_rate==5)
		game.global.score=game.global.score+5;
		else if(spawn_rate<30)
		game.global.score=game.global.score+3;
		else if(spawn_rate<50)
		game.global.score=game.global.score+2;
		else
