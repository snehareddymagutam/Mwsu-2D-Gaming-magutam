...
Level.prototype.create = function () {
	...
	{ 
        var max=3400;
        var min=30;
        var ex=Math.random() * (max - min) + min;
        var ey=Math.random() * (max - min) + min;
    ...
    var x,y,l,m,p3x,p3y,p4x,p4y,p5x,p5y,bx,by;
    ...
    if(game.global.current_level=="level_05"){
        ...
        p3x=1579;
        p3y=1294;
        bx=game.camera.width / 2 + 200;
        by=game.camera.height / 2 + 500;
    } 

    this.backportal= game.add.sprite(bx, by, 'blue_portal');
    this.backportal.animations.add('rotate', Phaser.Animation.generateFrameNames('blue_portal', 1, 3), 60, true);
